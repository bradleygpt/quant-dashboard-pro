...
def score_universe_pit(target_date, universe_tickers, sector_relative=True,
    ...
    # EPS Revisions pillar.
    #
    # VERSION B: For EPS Revisions pillar (PEAD), missing data is treated as
    # NEUTRAL (50th percentile, grade C) instead of bottom (grade F). This avoids
    # systematically penalizing tickers with sparse early-period earnings data.
    pillar_scores = {}

    for pillar_name, metrics in PIT_PILLAR_METRICS.items():
        pillar_metric_scores = []
        # Treat missing PEAD as neutral, not bottom-bias
        is_eps_revisions = (pillar_name == "EPS Revisions")
        ...
        for yf_key, display_name, higher_is_better in metrics:
            ...
            if sector_relative and "sector" in df.columns:
                if higher_is_better:
                    pct = col.groupby(df["sector"]).rank(pct=True, na_option="keep") * 100
                else:
                    pct = (1 - col.groupby(df["sector"]).rank(pct=True, na_option="keep")) * 100
            else:
                if higher_is_better:
                    pct = col.rank(pct=True, na_option="keep") * 100
                else:
                    pct = (1 - col.rank(pct=True, na_option="keep")) * 100

            # Version B: For EPS Revisions, fill missing with 50 (neutral). Other pillars use bottom.
            if is_eps_revisions:
                pct = pct.fillna(50)
            else:
                pct = pct.fillna(0)  # Bottom percentile for missing fundamentals (default behavior)

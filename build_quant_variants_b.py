...
# Configuration
RESULTS_FILE = "quant_variants_results_b.json"
...
def score_universe_at_date(checkpoint_date, universe):
    ...
    # Use the new PIT scoring (Version B: PEAD with neutral fallback for missing data)
    from pit_scoring_b import score_universe_pit
...
def main():
    ...
    # Version B: Start 2011-Q1 because 5-pillar scoring needs prior-year data for Growth and PEAD
    # (EDGAR XBRL data starts 2009-Q1, so 2011-Q1 has the full 2-year history needed)
    checkpoint_dates = get_first_of_months(start_year=2011)
    print(f"Backtesting {len(checkpoint_dates)} quarterly checkpoints (Version B: 5-pillar PEAD-neutral, 2011 start)", flush=True)
    if LOCAL_RUN:
